                ...
                <table cellspacing="0" class="basic-table">
                    <tr class="header-row">
                        ...
                        <#if itemsAvailableToReceive>
                            ...
                            <td>${uiLabelMap.ProductPackaging}</td>
                    ...
                    <#list orderItemDatas?if_exists as orderItemData>
                        ...
                        <tr id="orderItemData_tableRow_${rowCount}" valign="middle"<#if alt_row> class="alternate-row"</#if>>
                            ...
                            <td>${orderItem.quantity}<#if productUom?has_content> ${productUom.get("abbreviation",locale)?default(productUom.uomId)}</#if></td>
                            <td>${orderItem.cancelQuantity?default(0)}<#if productUom?has_content> ${productUom.get("abbreviation",locale)?default(productUom.uomId)}</#if></td>
                            <td>
                                <div ${(backOrderedQuantity &gt; 0)?string(" errorMessage","")}">
                                    ${backOrderedQuantity}<#if productUom?has_content> ${productUom.get("abbreviation",locale)?default(productUom.uomId)}</#if>
                                </div>
                            </td>
                            <td>${totalQuantityReceived}<#if productUom?has_content> ${productUom.get("abbreviation",locale)?default(productUom.uomId)}</#if></td>
                            <td>${orderItem.quantity - orderItem.cancelQuantity?default(0) - totalQuantityReceived}<#if productUom?has_content> ${productUom.get("abbreviation",locale)?default(productUom.uomId)}</#if></td>
                            <td>
                                <div>
                            ...
                            <#if availableToReceive &gt; 0 >
                                <td>
                                    ...
                                    <#if orderItem.altQuantity?has_content>
                                    <input type="hidden" name="altUnitCost_o_${rowCount}" value="${orderItem.altUnitPrice?default(0)}"/>
                                    </#if>
                                ...
                                <#if orderItem.altQuantity?has_content>
                                    <td align="right">
                                        <input type="hidden" id="piecesIncluded_${rowCount}" name="piecesIncluded_o_${rowCount}" value="${piecesIncluded}"/>
                                    ...
                                    <script language="JavaScript" type="text/javascript">
                                        function selectQuantityUom (e, rowCount) {
                                            ...
                                            var currentQuantity = $('#quantityAccepted_o_'+rowCount).val();
                                            jQuery.ajax({
                                                url: "getConvertedQuantityAndPrice",
                                                data: { currentPiecesIncluded : currentPiecesIncluded, newPackagingUnit : newPackagingUnit, currentQuantity : currentQuantity},
                                                async: true,
                                                type: "POST",
                                                success: function(data){
                                                    if (data.convertedQuantity) {
                                                        ...
                                                        $('#quantityAccepted_o_'+rowCount).val(qty) ;
                                                    }
                                                }
                                            });
                                        }
