    ...
    <#if orderItemDatas?exists>
        ...
        <table cellspacing="0" cellpadding="2" class="basic-table hover-bar">
            ...
            <#list orderItemDatas?if_exists as orderItemData>
                ...
                <#assign item = delegator.findOne("OrderItem", false, Static["org.ofbiz.base.util.UtilMisc"].toMap("orderId", orderItemAndShipGroupAssoc.orderId, "orderItemSeqId", orderItemAndShipGroupAssoc.orderItemSeqId ))?if_exists/>
                <#if item.altQuantity?has_content>
                    <#assign piecesIncluded = Static["org.ofbiz.order.order.OrderReadHelper"].getAlternativeConversionFactor(item)>
                    <#assign quantityUom = item.getRelatedOne("QuantityUom")?if_exists>
                    <#assign productForUom = item.getRelatedOne("Product")?if_exists/>
                    ...
                    <td>
                        <#if itemIssuances?has_content>
                            <#list itemIssuances as itemIssuance>
                                <div><b>[${itemIssuance.quantity?if_exists / piecesIncluded}<#if quantityUom?has_content> ${quantityUom.get("abbreviation",locale)?default(quantityUom.uomId)}</#if> <#if productUom?has_content>(${itemIssuance.quantity?if_exists} ${productUom.get("abbreviation",locale)?default(productUom.uomId)})</#if>]</b><br>${itemIssuance.shipmentId?if_exists}:${itemIssuance.shipmentItemSeqId?if_exists} ${uiLabelMap.CommonOn} [${(itemIssuance.issuedDateTime.toString())?if_exists}] ${uiLabelMap.CommonBy} [${(itemIssuance.issuedByUserLoginId)?if_exists}]</div>
                            </#list>
                        <#else>
                            <div>&nbsp;</div>
                        </#if>
                        ...
                        <div>
                            <#if isSalesOrder>
                                ...
                                <span>
                                    ...
                                    [${totalQuantityIssued/piecesIncluded} + ${totalQuantityReserved/piecesIncluded} = ${totalQuantityIssuedAndReserved/piecesIncluded} <#if quantityUom?has_content> ${quantityUom.get("abbreviation",locale)?default(quantityUom.uomId)}</#if><#if productUom?has_content> (${totalQuantityIssuedAndReserved} ${productUom.get("abbreviation",locale)?default(productUom.uomId)})</#if>]
                                    <b>
                                        ...
                                        ${orderItemAndShipGroupAssoc.quantity /piecesIncluded}<#if quantityUom?has_content> ${quantityUom.get("abbreviation",locale)?default(quantityUom.uomId)}</#if><#if productUom?has_content> (${orderItemAndShipGroupAssoc.quantity} ${productUom.get("abbreviation",locale)?default(productUom.uomId)})</#if>
                                    </b>
                                </span>
                            <#else>
                                ...
                                <span>
                                    ...
                                    ${totalQuantityIssued/piecesIncluded}<#if quantityUom?has_content> ${quantityUom.get("abbreviation",locale)?default(quantityUom.uomId)}</#if><#if productUom?has_content> (${totalQuantityIssued} ${productUom.get("abbreviation",locale)?default(productUom.uomId)})</#if>
                                    <b>
                ...
                <#if isSalesOrder>
                    <#list orderItemShipGrpInvResDatas as orderItemShipGrpInvResData>
                        ...
                        <tr id="orderItemData_tableRow_${rowCount}">
                            ...
                            <td>&nbsp;</td>
                            <td>${orderItemShipGrpInvRes.quantity}<#if productUom?has_content> ${productUom.get("abbreviation",locale)?default(productUom.uomId)}</#if></td>
                            <td>${orderItemShipGrpInvRes.quantityNotAvailable?default("&nbsp;")}<#if orderItemShipGrpInvRes.quantityNotAvailable?has_content && productUom?has_content> ${productUom.get("abbreviation",locale)?default(productUom.uomId)}</#if></td>
                            <#if originFacility?exists && originFacility.facilityId == inventoryItem.facilityId?if_exists>
                                ...
                                <#if productUom?has_content> ${productUom.get("abbreviation",locale)?default(productUom.uomId)}</#if>
                                </td>
                                <td align="right">
                                  <input type="checkbox" name="_rowSubmit_o_${rowCount}" value="Y" onclick="javascript:checkToggle(this, 'selectAllForm');highlightRow(this,'orderItemData_tableRow_${rowCount}');" />
                                </td>
