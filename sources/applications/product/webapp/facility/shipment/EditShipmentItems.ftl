    ...
    <div class="screenlet-body">
        ...
        <#list shipmentItemDatas as shipmentItemData>
            ...
            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
                ...
                <td>${shipmentItem.quantity?default("&nbsp;")}<#if productUom?has_content> ${productUom.abbreviation}</#if></td>
            ...
            <#list orderShipments as orderShipment>
                ...
                <#assign item = delegator.findOne("OrderItem", false, Static["org.ofbiz.base.util.UtilMisc"].toMap("orderId", orderShipment.orderId?if_exists, "orderItemSeqId", orderShipment.orderItemSeqId?if_exists))?if_exists/>
                ...
                <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
                    ...
                    <td>&nbsp;</td>
                    <td>${orderShipment.quantity?if_exists}<#if productUom?has_content> ${productUom.abbreviation}</#if></td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
            ...
            <#list itemIssuances as itemIssuance>
                <#assign piecesIncluded = 1.0>
                <#assign item = delegator.findOne("OrderItem", false, Static["org.ofbiz.base.util.UtilMisc"].toMap("orderId", itemIssuance.orderId?if_exists, "orderItemSeqId", itemIssuance.orderItemSeqId?if_exists))?if_exists/>
                <#if item.altQuantity?has_content>
                    <#assign piecesIncluded = Static["org.ofbiz.order.order.OrderReadHelper"].getAlternativeConversionFactor(item)>
                    <#assign quantityUom = item.getRelatedOne("QuantityUom")?if_exists>
                </#if>
                <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
                    ...
                    <td>${itemIssuance.quantity?if_exists}<#if productUom?has_content> ${productUom.abbreviation}</#if></td>
            ...
            <#list shipmentPackageContents as shipmentPackageContent>
                <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
                    ...
                    <td>${shipmentPackageContent.quantity?if_exists}<#if productUom?has_content> ${productUom.abbreviation}</#if></td>
                    ...
                    <td>${shipmentPackageContent.subProductQuantity}<#if productUom?has_content> ${productUom.abbreviation}</#if></td>
                    <#else>
                    <td colspan="2">&nbsp;</td>
                    </#if>
                ...
                <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
                    ...
                    <td>
                        <div>
                            ...
                            <#if productUom?has_content> ${productUom.abbreviation}</#if>
