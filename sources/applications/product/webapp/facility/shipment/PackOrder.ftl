                  ...
                  <table class="basic-table" cellspacing='0'>
                    ...
                    <#if (itemInfos?has_content)>
                      ...
                      <#-- <#list itemInfos as orderItem>  -->
                        ...
                        <#assign item = delegator.findOne("OrderItem", false, Static["org.ofbiz.base.util.UtilMisc"].toMap("orderId", orderItem.orderId, "orderItemSeqId", orderItem.orderItemSeqId )?if_exists)/>
                        <#assign piecesIncluded = 1.0>
                        <#if item?has_content && item.altQuantity?has_content>
                            <#assign piecesIncluded = orderReadHelper.getAlternativeConversionFactor(item)>
                        ...
                        <tr>
                          ...
                          <td align="right">${orderItemQuantity / piecesIncluded}<#if quantityUom?has_content> ${quantityUom.get("abbreviation",locale)?default(quantityUom.uomId)}</#if> <#if productUom?has_content>(${orderItemQuantity} ${productUom.get("abbreviation",locale)?default(productUom.uomId)})</#if></td>
                          <td align="right">${shippedQuantity?default(0) / piecesIncluded}<#if quantityUom?has_content> ${quantityUom.get("abbreviation",locale)?default(quantityUom.uomId)}</#if> <#if productUom?has_content>(${shippedQuantity?default(0)} ${productUom.get("abbreviation",locale)?default(productUom.uomId)})</#if></td>
                          <td align="right">${packingSession.getPackedQuantity(orderId, orderItem.orderItemSeqId, shipGroupSeqId, itemInfo.productId) / piecesIncluded}<#if quantityUom?has_content> ${quantityUom.get("abbreviation",locale)?default(quantityUom.uomId)}</#if> <#if productUom?has_content>(${packingSession.getPackedQuantity(orderId, orderItem.orderItemSeqId, shipGroupSeqId, itemInfo.productId)} ${productUom.get("abbreviation",locale)?default(productUom.uomId)})</#if></td>
                          <td>&nbsp;</td>
                          <td align="center">
                ...
                <table class="basic-table" cellspacing='0'>
                  ...
                  <#list packedLines as line>
                    ...
                    <tr>
                      ...
                      <#assign item = delegator.findOne("OrderItem", false, Static["org.ofbiz.base.util.UtilMisc"].toMap("orderId", line.getOrderId(), "orderItemSeqId", line.getOrderItemSeqId() )?if_exists)/>
                      <#assign piecesIncluded = 1.0>
                      <#if item?has_content && item.altQuantity?has_content>
                          <#assign piecesIncluded = orderReadHelper.getAlternativeConversionFactor(item)>
                          <#assign quantityUom = item.getRelatedOne("QuantityUom")?if_exists>
                      ...
                      <td align="right">${line.getQuantity()/ piecesIncluded}<#if quantityUom?has_content> ${quantityUom.get("abbreviation",locale)?default(quantityUom.uomId)}</#if> <#if productUom?has_content>(${line.getQuantity()} ${productUom.get("abbreviation",locale)?default(productUom.uomId)})</#if></td>
