          ...
          <table class="basic-table" cellspacing="0">
            ...
            <#list receivedItems as item>
                <#if item.productId?has_content>
                    <#assign productValue = delegator.findOne("Product", {"productId" : item.productId}, false)?if_exists>
                    <#if productValue?has_content && productValue.quantityUomId?has_content>
                        <#--Retrieve qty uom from product -->
                        <#assign productUom = productValue.getRelatedOne("QuantityUom")?if_exists>
                    </#if>
                </#if>
              <form name="cancelReceivedItemsForm_${item_index}" method="post" action="<@ofbizUrl>cancelReceivedItems</@ofbizUrl>">
                ...
                <tr>
                  ...
                  <td>${item.quantityRejected?default(0)?string.number}<#if productUom?has_content> ${productUom.get("abbreviation",locale)?default(productUom.uomId)}</#if></td>
                  <td>${item.quantityAccepted?string.number}<#if productUom?has_content> ${productUom.get("abbreviation",locale)?default(productUom.uomId)}</#if></td>
                  <td>
              ...
              <tr>
                ...
                <td width="74%">
                  ...
                  <#if product.quantityUomId?exists>
                      <#assign uom = delegator.findOne("Uom", {"uomId" : product.quantityUomId}, false)?if_exists>
                      <input type="hidden" name="packagingUnit" value="${altQuantityUomId!}"/>
              ...
              <#else/>
                ...
                <#list purchaseOrderItems as orderItem>
                  ...
                  <#if orderItem.altQuantity?has_content>
                      <#assign piecesIncluded = orderItem.quantity / orderItem.altQuantity>
                  ...
                  <#assign defaultPiecesIncluded = parameters.get("piecesIncluded_o_" + rowCount)?default(piecesIncluded)>
                  <input type="hidden" id="piecesIncluded_${rowCount}" name="piecesIncluded_o_${rowCount}" value="${defaultPiecesIncluded}"/>
                    ...
                    <td>
                      <table class="basic-table" cellspacing="0">
                        <tr>
                          ...
                          <td align="right">
                            <input type="text" id="quantityAccepted_${rowCount}" name="quantityAccepted_o_${rowCount}" size="6" value=<#if partialReceive?exists>"0"<#else>"${(defaultQuantity / piecesIncluded)?string.number}"</#if>/>
                          </td>
                          <#if orderItem.altQuantity?has_content>
                          <td align="right">
                              <select id="packagingUnit_${rowCount}" name="packagingUnit_o_${rowCount}" size="1" onchange="javascript:selectQuantityUom(this, '${rowCount}');">
                                  <#-- packaging unit-->
                                  <#if quantityUom?has_content>
                                  <option value="${quantityUom.uomId}" id="${piecesIncluded!}"<#if quantityUom.uomId == parameters.get("packagingUnit_o_" + rowCount)?default(orderItem.quantityUomId) >selected=true</#if>>
                                      ${quantityUom.get("description",locale)?default(quantityUom.uomId)}
                                  </option>
                                  </#if>
                                  <#-- stock unit-->
                                  <#if productUom?has_content>
                                  <option value="${productUom.uomId}" id="1" <#if productUom.uomId == parameters.get("packagingUnit_o_" + rowCount)?default(orderItem.quantityUomId) >selected=true</#if>>
                                      ${productUom.get("description",locale)?default(productUom.uomId)}
                                  </option>
                                  </#if>
                              </select>
                          </td>
                          ...
                          <script language="JavaScript" type="text/javascript">
                              function selectQuantityUom (e, rowCount) {
                                var currentPiecesIncluded = $('#piecesIncluded_'+rowCount).val();
                                var newPackagingUnit = $('#packagingUnit_'+rowCount+" option:selected").attr("id");
                                $('#piecesIncluded_'+rowCount).val(newPackagingUnit);

                                var currentQuantity = $('#quantityAccepted_'+rowCount).val();
                                var currentRejectedQuantity = $('#quantityRejected_'+rowCount).val();
                                var currentUnitCost = $('#unitCost_'+rowCount).val();
                                jQuery.ajax({
                                    url: "getConvertedQuantityAndPrice",
                                    data: { currentPiecesIncluded : currentPiecesIncluded, newPackagingUnit : newPackagingUnit, currentQuantity : currentQuantity, currentRejectedQuantity : currentRejectedQuantity, currentUnitCost : currentUnitCost},
                                    async: true,
                                    type: "POST",
                                    success: function(data){
                                        if (data.convertedQuantity) {
                                            var qty = data.convertedQuantity.toString();
                                            if (qty.indexOf(".") != null) {
                                                qty = qty.replace('.',',');
                                            }
                                            $('#quantityAccepted_'+rowCount).val(qty) ;
                                        }
                                        if (data.convertedRejectedQuantity) {
                                            var qty = data.convertedRejectedQuantity.toString();
                                            if (qty.indexOf(".") != null) {
                                                qty = qty.replace('.',',');
                                            }
                                            $('#quantityRejected_'+rowCount).val(qty) ;
                                        }
                                        if (data.convertedUnitCost) {
                                            var cost = data.convertedUnitCost.toString();
                                            if (cost.indexOf(".") != null) {
                                                cost = cost.replace('.',',');
                                            }
                                           $('#unitCost_'+rowCount).val(cost) ;
                                        }
                                    }
                                });
                              }
                        ...
                        <tr>
                          ...
                          <td align="right">
                            <input type="text" id="quantityRejected_${rowCount}" name="quantityRejected_o_${rowCount}" value="0" size="6"/>
                          </td>
                          <tr>
                            <td>&nbsp;</td>
                            ...
                            <td align="right">
                              <input type="text" class="inputBox" name="quantityOrdered" value="${orderItem.quantity / piecesIncluded}" size="6" maxlength="20" disabled="disabled" />
                              <#if quantityUom?has_content>
                                  ${quantityUom.get("description",locale)?default(quantityUom.uomId)}
                              </#if>
                            </td>
                        ...
                        <tr>
                          ...
                          <#else>
                            <#if orderItem.altUnitPrice?exists>
                                <#assign itemCost = orderItem.altUnitPrice?default(0)/>
                            </#if>
                            <td align="right">${uiLabelMap.ProductPerUnitPrice}:</td>
                            <td align="right">
                              <input type="hidden" name="currencyUomId_o_${rowCount}" value="${currencyUomId?if_exists}" />
                              <input type="text" id="unitCost_${rowCount}" name="unitCost_o_${rowCount}" value="${itemCost}" size="6" maxlength="20" />
                              ${currencyUomId?if_exists}
                            </td>
                          </#if>
                        </tr>
