      ...
      <#list returnableItems.keySet() as orderItem>
        ...
        <#else>
            ...
            <input type="hidden" name="description_o_${rowCount}" value="${orderItem.itemDescription?if_exists}"/>
              ...
              <td align='center'>
                <div>${orderItem.quantity?string.number}<#if productUom?has_content> ${productUom.abbreviation?default(productUom.uomId)}</#if> (${(orderItem.quantity?default(0)/ piecesIncluded)?string.number} <#if quantityUom?has_content> ${quantityUom.abbreviation?default(quantityUom.uomId)}</#if>)</div>
              </td>
              <td>
                <input type="text" size="6" name="returnQuantity_o_${rowCount}" value="${returnableItems.get(orderItem).get("returnableQuantity")}"/>
                <#if productUom?has_content> ${productUom.abbreviation?default(productUom.uomId)}</#if>
              </td>
              <td align='left'>
