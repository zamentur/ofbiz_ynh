...
if (product) {
    ...
    // get the product uom information
    List altQuantityUomIds = request.getAttribute("altQuantityUomIds");
    if (UtilValidate.isEmpty(altQuantityUomIds)) {
        ...
        if(product != null){
            altQuantityUomIds = FastList.newInstance();
    ...
    if (UtilValidate.isNotEmpty(altQuantityUomIds)) {
        context.altQuantityUomIds = altQuantityUomIds;
    }
    ...
    if ("Y".equals(product.isVirtual)) {
                    ...
                    if (variants) {
                        ...
                        variants.each { variantAssoc ->
                            ...
                            if (cart.isSalesOrder()) {
                                ...
                                variantPriceMap.put("minimumQuantity", ShoppingCart.getMinimumOrderQuantity(delegator, calculatedPrice, variant.get("productId"), null));
                            ...
                            if(virtualVariants){
                                virtualVariants.each { virtualAssoc ->
                                    ...
                                    if (cart.isSalesOrder()) {
                                        ...
                                        // Get the minimum quantity for variants if MINIMUM_ORDER_PRICE is set for variants.
                                        virtualPriceMap.put("minimumQuantity", ShoppingCart.getMinimumOrderQuantity(delegator, calculatedPrice, virtual.get("productId"), null));
                                        Iterator treeMapIter = variantTree.entrySet().iterator();
                                        while (treeMapIter.hasNext()) {
                                            Map.Entry entry = treeMapIter.next();
                                            if (entry.getValue() instanceof  Map) {
        ...
        context.minimumQuantity= ShoppingCart.getMinimumOrderQuantity(delegator, priceMap.price, productId, null);
        if(isAlternativePacking){
