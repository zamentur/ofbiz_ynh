...
<script language="JavaScript" type="text/javascript">
  ...
  function checkProductAlternativeUoms(e) {
        var selectedValue = e.value;
        if (selectedValue == "") {
            return;
        }
        var orderValue = document.appendItemForm.orderId.value;
        if (orderValue == "" || orderValue == undefined) {
            return;
        }
        var quantityValue = document.appendItemForm.quantity.value;
        //Check if product exists and has defined quantityUomId
        jQuery.ajax({
            url: '/ordermgr/control/checkOrderProductAlternativeQuantity',
            async: false,
            type: 'POST',
            data: {"productId" : selectedValue, "quantity" : quantityValue, "orderId": orderValue},
            success: function(data) {
                jQuery('#showQuantity').html(data);
            }
        });
    }

    function checkProductAlternativeUomsForQuantity(e) {
        var selectedValue = e.value;
        if (selectedValue == "") {
            return;
        }
        ...
        var productValue = document.appendItemForm.productId.value;
        if (productValue == "" || productValue == undefined) {
            return;
        }
        //Check if product exists and has 
        jQuery.ajax({
            url: '/ordermgr/control/checkOrderProductAlternativeQuantity',
            async: false,
            type: 'POST',
            data: {"productId" : productValue, "quantity" : selectedValue, "orderId": orderValue},
            success: function(data) {
                jQuery('#showQuantity').html(data);
            }
        });
              ...
              <#if catalogCol?has_content && (catalogCol?size > 1)>
                ...
                <tr>
                  ...
                  <td>
                      ...
                      <@htmlTemplate.lookupField formName="appendItemForm" name="productId" id="productId" fieldFormName="LookupProduct" event="onBlur" action="javascript:checkProductAlternativeUoms(this);"/>
                ...
                <tr>
                  ...
                  <td>
                      <div>
                      <input type="text" size="6" name="quantity" value="${requestParameters.quantity?default("1")}" onblur="javascript:checkProductAlternativeUomsForQuantity(this);"/>
                          <span id="showQuantity">
                          <#if altQuantityUomIds?exists>
                              <#if (altQuantityUomIds.size() > 1)>
                              <select name="quantityUomId">
                              ...
                              <#else>
                                  ...
                                  <#assign uom = delegator.findOne("Uom", {"uomId" : altQuantityUomId}, false)?if_exists>
                                  <input type="hidden" name="quantityUomId" value="${altQuantityUomId!}"/>
