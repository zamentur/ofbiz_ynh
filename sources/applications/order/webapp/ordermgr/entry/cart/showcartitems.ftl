    ...
    <form method="post" action="<@ofbizUrl>modifycart</@ofbizUrl>" name="cartform" style="margin: 0;">
      ...
      <table cellspacing="0" cellpadding="1" border="0">
        ...
        <#list shoppingCart.items() as cartLine>
          ...
          <#if cartLine.useAlternaltiveUomQty()>
              <#assign quantityUom = cartLine.getQuantityUom()!>
              <#assign product = cartLine.getProduct()!>
              <#if product?has_content>
                  <#assign productUom = product.getRelatedOne("QuantityUom")?if_exists>
              </#if>
          </#if>
          ...
          <tr><td colspan="2">
              ...
              <tr>
                <td colspan="2">
                  <div>
                    ...
                    <#if cartLine.useAlternaltiveUomQty()>
                        <#if cartLine.getAlternativeProductQuantity()?has_content>
                             <#assign piecesIncluded =  cartLine.getAlternativeProductQuantity() / cartLine.getQuantity()>
                        </#if>
                        <#assign atpQty = availableToPromiseMap.get(productId)>
                        <#assign atpAlternativeQty = Static["org.ofbiz.common.uom.UomWorker"].convertUom(atpQty, piecesIncluded!, 4, 2, cartLine.getQuantityUomId(), cartLine.getProduct().getString("quantityUomId"), dispatcher)>
                        <#assign qohQty = quantityOnHandMap.get(productId)>
                        <#assign qohAlternativeQty = Static["org.ofbiz.common.uom.UomWorker"].convertUom(qohQty, piecesIncluded!, 4, 2, cartLine.getQuantityUomId(), cartLine.getProduct().getString("quantityUomId"), dispatcher)>
                        ${uiLabelMap.ProductAtp} = ${atpQty!} <#if productUom?has_content>${productUom.get("description",locale)?default(productUom.uomId)}</#if> / ${atpAlternativeQty!} <#if quantityUom?has_content> ${quantityUom.get("description",locale)?default(quantityUom.uomId)}</#if>,
                        ${uiLabelMap.ProductQoh} = ${qohQty!} <#if productUom?has_content>${productUom.get("description",locale)?default(productUom.uomId)}</#if> / ${qohAlternativeQty!} <#if quantityUom?has_content> ${quantityUom.get("description",locale)?default(quantityUom.uomId)}</#if>
                    <#else>
            ...
            <td nowrap="nowrap" align="center">
              <div>
                ...
                <#else>
                    <#if cartLine.useAlternaltiveUomQty()>
                        <input size="6" type="text" name="update_${cartLineIndex}" value="${cartLine.getAlternativeProductQuantity()?string.number}"/><#if quantityUom?has_content> ${quantityUom.get("description",locale)?default(quantityUom.uomId)}</#if>
                        <br/>${cartLine.getQuantity()} <#if productUom?has_content>${productUom.get("description",locale)?default(productUom.uomId)}</#if>
                    <#else>
            ...
            <td nowrap="nowrap" align="right">
              <div>
                ...
                <#else>
                    ...
                    <#else>
                        <#if cartLine.useAlternaltiveUomQty()>
                            <#assign price = cartLine.getAlternativeProductPrice()>
                        <#else>
                    ...
                    <#if cartLine.useAlternaltiveUomQty()>
                        <#assign elementPrice = cartLine.getBasePrice() >
                        <br/> <@ofbizCurrency amount=elementPrice isoCode=currencyUomId rounding=3/>
                    </#if>
                </#if>
              </div>
            </td>
