  ...
  <div class="screenlet-body">
    <table width="100%">
      ...
      <tr>
        ...
        <#if shoppingCartItem.useAlternaltiveUomQty()>
            <#assign piecesIncluded = 1.0>
            <#if shoppingCartItem.getAlternativeProductQuantity()?has_content>
                <#assign piecesIncluded = shoppingCartItem.getQuantity() / shoppingCartItem.getAlternativeProductQuantity()>
            </#if>
            <#assign quantityUom = shoppingCartItem.getQuantityUom()!>
            <#assign product = shoppingCartItem.getProduct()!>
            <#if product?has_content>
                <#assign productUom = product.getRelatedOne("QuantityUom")?if_exists>
            </#if>
        </#if>
        <td valign="top"> <#if shoppingCartItem.useAlternaltiveUomQty()>${(cartShipInfo.getShipItemInfo(shoppingCartItem).getItemQuantity()?default("0") / piecesIncluded)?string.number}<#if quantityUom?has_content> ${quantityUom.get("description",locale)?default(quantityUom.uomId)}</#if><#else>${cartShipInfo.getShipItemInfo(shoppingCartItem).getItemQuantity()?default("0")}</#if> </td>
