        ...
        try {
            ...
            for(GenericValue requirement : requirementAndRoles) {
                ...
                if (supplierProduct == null) {
                    ...
                    //Check quantityUomId
                    String uomId = supplierProduct.getString("quantityUomId");
                    if (uomId != null) {
                        GenericValue product = delegator.findOne("Product", false, UtilMisc.toMap("productId", productId));
                        String uomIdTo = null;
                        if (UtilValidate.isNotEmpty(product)) {
                            uomIdTo = product.getString("quantityUomId");
                        }
                         //Check minimum
                        BigDecimal minimumOrderQuantity = supplierProduct.getBigDecimal("minimumOrderQuantity");
                        if (uomId.equals(uomIdTo) == false) {
                            //convert quantity
                            String roundingModeStr = null;
                            if (UtilValidate.isNotEmpty(supplierProduct.get("roundingMode"))) {
                                roundingModeStr = supplierProduct.getString("roundingMode");
                            }
                            Long scaleLong = null;
                            if (UtilValidate.isNotEmpty(supplierProduct.get("decimalScale"))) {
                                scaleLong = supplierProduct.getLong("decimalScale");
                            }
                            BigDecimal conversionFactor = null;
                            if (UtilValidate.isNotEmpty(supplierProduct.get("piecesIncluded"))) {
                                conversionFactor = supplierProduct.getBigDecimal("piecesIncluded");
                            }
                            BigDecimal minimumQuantity = UomWorker.convertStockUom(minimumOrderQuantity, conversionFactor, roundingModeStr, scaleLong, uomId, uomIdTo, dispatcher);
                            if (requiredQuantity.compareTo(minimumQuantity) < 0) {
                                requiredQuantity = minimumOrderQuantity;
                            } else {
                                if (UtilValidate.isNotEmpty(conversionFactor)) {
                                    Integer roundingMode = UtilNumber.getBigDecimalRoundingMode("stock.rounding");
                                    if (UtilValidate.isNotEmpty(roundingModeStr)) {
                                        roundingMode = UtilNumber.roundingModeFromString(roundingModeStr);
                                    }
                                    Integer decimalScale = UtilNumber.getBigDecimalScale("stock.decimals");
                                    if (UtilValidate.isNotEmpty(scaleLong)) {
                                        decimalScale = Integer.valueOf(scaleLong.intValue());
                                    }
                                    requiredQuantity = requiredQuantity.divide(conversionFactor, decimalScale, roundingMode);
                                } else {
                                    requiredQuantity = UomWorker.convertStockUom(requiredQuantity, conversionFactor, roundingModeStr, scaleLong, uomIdTo, uomId, dispatcher);
                                }

                                if (UtilValidate.isEmpty(conversionFactor)) {
                                    //Retrieve conversionFactor from UomConversion
                                    GenericValue uomConversion = delegator.findOne("UomConversion", UtilMisc.toMap("uomId", uomIdTo, "uomIdTo", uomId), false);
                            ...
                            requirement.put("quantity", requiredQuantity);
                            union.put("productQuantityUomId", uomId);
                            union.put("quantityUomId", uomIdTo);
                        }
                    }
                }
