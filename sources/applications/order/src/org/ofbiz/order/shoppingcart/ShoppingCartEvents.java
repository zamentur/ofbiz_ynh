...
public class ShoppingCartEvents {
    ...
    public static String addToCart(HttpServletRequest request, HttpServletResponse response) {
            ...
            if (!configWrapper.isCompleted()) {
                try {
                    ...
                    if (UtilValidate.isNotEmpty(product)) {
                        ...
                        if (UtilValidate.isEmpty(productQuantityUomId)) {
                            request.setAttribute("paramMap", paramMap);
                            request.setAttribute("productId", productId);
                            request.setAttribute("_EVENT_MESSAGE_", UtilProperties.getMessage(resource_error, "cart.addToCart.enterProductQuantityUomIdBeforeAddingToCart", locale));
                            return "success";
                        }
                    }
                } catch (GenericEntityException e) {
        ...
        //Check if we have a different uom between stock and order
        String quantityUomId = (String) paramMap.remove("quantityUomId");
        if (UtilValidate.isEmpty(quantityUomId) && UtilValidate.isNotEmpty(configWrapper) && configWrapper.isCompleted()) {
            request.setAttribute("paramMap", paramMap);
            request.setAttribute("productId", productId);
            request.setAttribute("_EVENT_MESSAGE_", UtilProperties.getMessage(resource_error, "cart.addToCart.enterProductQuantityUomIdBeforeAddingToCart", locale));
            return "product";
        ...
        if (cart.getOrderType().equals("PURCHASE_ORDER")) {
            ...
            productPriceValues = cart.getSupplierProductList(productId, quantity, null, dispatcher);
        } else if (cart.getOrderType().equals("SALES_ORDER")) {
            //Check product prices
            productPriceValues = cart.getProductSalesProductPrices(productId, quantity, null, null, "PURCHASE", null, null, dispatcher);
        }
        if (UtilValidate.isNotEmpty(productPriceValues)) {
            //Filter by quantityUomId
            try {
                GenericValue product = delegator.findOne("Product", UtilMisc.toMap("productId", productId), false);
                if (UtilValidate.isNotEmpty(product)) {
                    String productQuantityUomId = product.getString("quantityUomId");
                    if (UtilValidate.isNotEmpty(productQuantityUomId)) {
                        productPriceValues = EntityUtil.filterByCondition(productPriceValues, EntityCondition.makeCondition("quantityUomId", EntityOperator.NOT_EQUAL, null));
                    }
                }
            } catch (GenericEntityException e) {
                e.printStackTrace();
        ...
        if (UtilValidate.isNotEmpty(productPriceValues) && UtilValidate.isEmpty(quantityUomId)) {
            //Prepare map for dropDown selection
            List<String> altQuantityUomIds = EntityUtil.getFieldListFromEntityList(productPriceValues, "quantityUomId", true);

            if (UtilValidate.isNotEmpty(altQuantityUomIds)) {
                request.setAttribute("paramMap", paramMap);
                request.setAttribute("productId", productId);
                request.setAttribute("altQuantityUomIds", altQuantityUomIds);
                request.setAttribute("_EVENT_MESSAGE_", UtilProperties.getMessage(resource_error, "cart.addToCart.chooseQuantityUomBeforeAddingToCart", locale));
                return "success";
            }
        }
        ...
        result = cartHelper.addToCart(catalogId, shoppingListId, shoppingListItemSeqId, productId, productCategoryId,
                ...
                shipBeforeDate, shipAfterDate, configWrapper, itemGroupNumber, paramMap, parentProductId, quantityUomId);
    ...
    public static String checkProductAlternativeQuantity(HttpServletRequest request, HttpServletResponse response) {
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        Locale locale = UtilHttp.getLocale(request);
        ...
        if (UtilValidate.isNotEmpty(quantityStr)) {
            ...
            try {
                ...
                Debug.logWarning(e, "Problem parsing quantity string: " + quantityStr, module);
                quantity = null;
            }
        } else {
            quantity = BigDecimal.ONE;
        ...
        try {
            GenericValue product = delegator.findOne("Product", UtilMisc.toMap("productId", productId), false);
            if(product != null){
                List<String> altQuantityUomIds = FastList.newInstance();
                LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
                ShoppingCart cart = ShoppingCartEvents.getCartObject(request);
                List<GenericValue> productPriceValues = new ArrayList<GenericValue>();
                if (cart.getOrderType().equals("PURCHASE_ORDER")) {
                ...
                if (UtilValidate.isNotEmpty(productPriceValues)) {
                    //Prepare map for dropDown selection
                    altQuantityUomIds = EntityUtil.getFieldListFromEntityList(productPriceValues, "quantityUomId", true);
                }
                request.setAttribute("altQuantityUomIds", altQuantityUomIds);
            }
        } catch (GenericEntityException e) {
