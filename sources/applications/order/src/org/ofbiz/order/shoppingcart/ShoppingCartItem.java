...
public class ShoppingCartItem implements java.io.Serializable {
    ...
    private transient GenericValue _quantityUom = null;
    private BigDecimal altProductQuantity = null;
    private BigDecimal altProductPrice = null;
    private BigDecimal altPiecesIncluded = null;
    private String altRoundingMode = null;
    private Long altDecimalScale = null;
    ...
    public static ShoppingCartItem makePurchaseOrderItem(Integer cartLocation, String productId, BigDecimal selectedAmount, BigDecimal quantity,
            ...
            LocalDispatcher dispatcher, ShoppingCart cart, GenericValue supplierProduct, Timestamp shipBeforeDate, Timestamp shipAfterDate, Timestamp cancelBackOrderDate, String quantityUomId)
        ...
        Boolean useAlternaltiveUomQty = false;
        if (UtilValidate.isNotEmpty(quantityUomId)) {
            try {
                String productQuantityUomId = product.getString("quantityUomId");
                if (UtilValidate.isNotEmpty(quantityUomId) && quantityUomId.compareTo(productQuantityUomId) != 0) {
                    GenericValue uom = delegator.findOne("Uom", UtilMisc.toMap("uomId", quantityUomId), false);
                    if (UtilValidate.isNotEmpty(uom)) {
                        useAlternaltiveUomQty = true;
                    } else {
                        quantityUomId = null;
                        Debug.logWarning("Quantity uom [" + quantityUomId + "] not found", module);
                    }
                } else if (quantityUomId.compareTo(productQuantityUomId) == 0) {
                    quantityUomId = null;
                    Debug.logWarning("Quantity uom [" + quantityUomId + "] is the same as product stock uom ["+ productQuantityUomId+"]", module);
                } else if (UtilValidate.isEmpty(productQuantityUomId)) {
                    throw new CartItemModifyException(UtilProperties.getMessage(resource_error, "cart.addToCart.enterProductQuantityUomIdBeforeAddingToCart", cart.getLocale()));
                }
            } catch (GenericEntityException e) {
                Debug.logWarning(e.toString(), module);
                quantityUomId = null;
        ...
        ShoppingCartItem newItem = new ShoppingCartItem(product, additionalProductFeatureAndAppls, attributes, prodCatalogId, configWrapper, cart.getLocale(), itemType, itemGroup, null, quantityUomId);
        ...
        if ( useAlternaltiveUomQty) {
            String uomId = newItem.getQuantityUomId();
            String uomIdTo = newItem.getProduct().getString("quantityUomId");

            List<GenericValue> productPriceValues = new ArrayList<GenericValue>();
            if (cart.getOrderType().equals("PURCHASE_ORDER")) {
                //Check product suppliers
                productPriceValues = cart.getSupplierProductList(newItem.getProductId(), quantity, uomId, dispatcher);
            } else if (cart.getOrderType().equals("SALES_ORDER")) {
                //Check product prices
                productPriceValues = cart.getProductSalesProductPrices(newItem.getProductId(), quantity, null, null, "PURCHASE", uomId, null, dispatcher);
            }
            if (UtilValidate.isNotEmpty(productPriceValues)) {
                BigDecimal initialQuantity = quantity;
                GenericValue productPriceValue = EntityUtil.getFirst(productPriceValues);
                String roundingModeStr = null;
                if (UtilValidate.isNotEmpty(productPriceValue.get("roundingMode"))) {
                    roundingModeStr = productPriceValue.getString("roundingMode");
                    newItem.setAlternativeRoundingMode(roundingModeStr);
                }
                Long scaleLong = null;
                if (UtilValidate.isNotEmpty(productPriceValue.get("decimalScale"))) {
                    scaleLong = productPriceValue.getLong("decimalScale");
                    newItem.setAlternativeDecimalScale(scaleLong);
                }
                BigDecimal conversionFactor = null;
                if (UtilValidate.isNotEmpty(productPriceValue.get("piecesIncluded"))) {
                    BigDecimal priceConversionFactor = productPriceValue.getBigDecimal("piecesIncluded");
                    conversionFactor = priceConversionFactor;
                    newItem.setAlternativePiecesIncluded(priceConversionFactor);
                }
                quantity = UomWorker.convertStockUom(quantity, conversionFactor, roundingModeStr, scaleLong, uomId, uomIdTo, dispatcher);
                if (UtilValidate.isEmpty(quantity)) {
                    ...
                    throw new CartItemModifyException(UtilProperties.getMessage(resource_error, "item.product_quantity_conversion_error",UtilMisc.toMap("uomIdTo", uomIdTo,"uomId", uomId), cart.getLocale()));
                } else {
                    newItem.setAlternativeProductQuantity(initialQuantity);
                }
            }
        }
        try {
        ...
        if (supplierProduct != null) {
            ...
            if ( useAlternaltiveUomQty) {
                // we need to divide the price by the quantity
                BigDecimal lastPrice = supplierProduct.getBigDecimal("lastPrice");
                BigDecimal pieces = BigDecimal.ONE;
                if (UtilValidate.isNotEmpty(newItem.getAlternativeConversionFactor())) {
                    pieces = newItem.getAlternativeConversionFactor();
                }
                BigDecimal dividedPrice = lastPrice.divide(pieces, UtilNumber.getBigDecimalScale("order.decimals"),UtilNumber.getBigDecimalRoundingMode("order.rounding"));
                newItem.setBasePrice(dividedPrice);
            }
        } else {
    ...
    /** Gets the Item Quantity Uom entity. If it is not already retrieved gets it from the delegator */
    public GenericValue getQuantityUom() {
        if (this._quantityUom != null) {
            return this._quantityUom;
        }
        if (this.quantityUomId != null) {
            try {
                this._quantityUom = this.getDelegator().findOne("Uom", UtilMisc.toMap("uomId", quantityUomId), true);
            } catch (GenericEntityException e) {
                throw new RuntimeException("Entity Engine error getting Product Quantity Uom (" + e.getMessage() + ")");
            }
        }
        return this._quantityUom;
    }
    ...
    public void setAlternativeProductPrice(BigDecimal altProductPrice) {
        this.altProductPrice = altProductPrice;
    }

    public BigDecimal getAlternativeProductPrice() {
        return altProductPrice;
    }
    
    public BigDecimal getAlternativeProductQuantity() {
        return altProductQuantity;
    }

    public void setAlternativeProductQuantity(BigDecimal altProductQuantity) {
        this.altProductQuantity = altProductQuantity;
    }

    public BigDecimal getAlternativePiecesIncluded() {
        return altPiecesIncluded;
    }

    public void setAlternativePiecesIncluded(BigDecimal altPiecesIncluded) {
        this.altPiecesIncluded = altPiecesIncluded;
    }

    public String getAlternativeRoundingMode() {
        return altRoundingMode;
    }

    public void setAlternativeRoundingMode(String altRoundingMode) {
        this.altRoundingMode = altRoundingMode;
    }

    public Long getAlternativeDecimalScale() {
        return altDecimalScale;
    }

    public void setAlternativeDecimalScale(Long altDecimalScale) {
        this.altDecimalScale = altDecimalScale;
    }

    public BigDecimal getAlternativeConversionFactor() {
        BigDecimal conversionFactor = null;
        BigDecimal piecesIncluded = getAlternativePiecesIncluded();
        if (UtilValidate.isEmpty(piecesIncluded)) {
            //Retrieve conversion factor from uom
            String uomId = this.getQuantityUomId();
            String uomIdTo = this.getProduct().getString("quantityUomId");
            try {
                GenericValue uomConversion = getDelegator().findOne("UomConversion", UtilMisc.toMap("uomId", uomId, "uomIdTo", uomIdTo), false);
                if (UtilValidate.isNotEmpty(uomConversion)) {
                    conversionFactor = uomConversion.getBigDecimal("conversionFactor");
                }
            } catch (GenericEntityException e) {
                e.printStackTrace();
            }
        } else {
            conversionFactor = piecesIncluded;
        }
        return conversionFactor;
    }

    public Boolean useAlternaltiveUomQty() {
    ...
    public static ShoppingCartItem makeItem(Integer cartLocation, String productId, BigDecimal selectedAmount, BigDecimal quantity, BigDecimal unitPrice,
            ...
            String itemType, ShoppingCart.ShoppingCartItemGroup itemGroup, LocalDispatcher dispatcher, ShoppingCart cart, Boolean triggerExternalOpsBool, Boolean triggerPriceRulesBool, String parentProductId, String quantityUomId, Boolean skipInventoryChecks, Boolean skipProductChecks)
            throws CartItemModifyException, ItemNotFoundException {
        ...
        return makeItem(cartLocation,productId,selectedAmount,quantity,unitPrice,
                ...
                parentProductId,quantityUomId,skipInventoryChecks,skipProductChecks);
    ...
    public static ShoppingCartItem makeItem(Integer cartLocation, String productId, BigDecimal selectedAmount, BigDecimal quantity, BigDecimal unitPrice,
        ...
        return makeItem(cartLocation, product, selectedAmount, quantity, unitPrice,
                ...
                additionalProductFeatureAndAppls, attributes, prodCatalogId, configWrapper,
                itemType, itemGroup, dispatcher, cart, triggerExternalOpsBool, triggerPriceRulesBool, parentProduct, quantityUomId, skipInventoryChecks, skipProductChecks);
    }
    ...
    public static ShoppingCartItem makeItem(Integer cartLocation, GenericValue product, BigDecimal selectedAmount,
            ...
            ShoppingCart cart, Boolean triggerExternalOpsBool, Boolean triggerPriceRulesBool, GenericValue parentProduct, String quantityUomId, Boolean skipInventoryChecks, Boolean skipProductChecks) throws CartItemModifyException {
        return makeItem(cartLocation,product,selectedAmount,
                ...
                triggerExternalOpsBool,triggerPriceRulesBool,parentProduct, quantityUomId,skipInventoryChecks,skipProductChecks);
    }
    ...
    */
    public static ShoppingCartItem makeItem(Integer cartLocation, GenericValue product, BigDecimal selectedAmount,
        ...
        ShoppingCartItem newItem = new ShoppingCartItem(product, additionalProductFeatureAndAppls, attributes, prodCatalogId, configWrapper, cart.getLocale(), itemType, itemGroup, parentProduct, quantityUomId);
    ...
    /** Creates new ShoppingCartItem object. */
    protected ShoppingCartItem(GenericValue product, Map<String, GenericValue> additionalProductFeatureAndAppls, Map<String, Object> attributes, String prodCatalogId, ProductConfigWrapper configWrapper, Locale locale, String itemType, ShoppingCart.ShoppingCartItemGroup itemGroup, GenericValue parentProduct, String quantityUomId) {
        this._product = product;
        this.productId = _product.getString("productId");
        this._parentProduct = parentProduct;
        if (parentProduct != null)
    ...
    public void updatePrice(LocalDispatcher dispatcher, ShoppingCart cart) throws CartItemModifyException {
        ...
        if (_product != null && isModifiedPrice == false) {
            try {
                ...
                if (useAlternaltiveUomQty()) {
                    priceContext.put("quantityUomId", getQuantityUomId());
                    if (UtilValidate.isNotEmpty(getAlternativeConversionFactor())) {
                        pieces = getAlternativeConversionFactor();
                    }
                    priceContext.put("quantity", this.getAlternativeProductQuantity());
                }
                ...
                if (cart.getOrderType().equals("PURCHASE_ORDER")) {
                    ...
                    } else if (useAlternaltiveUomQty()) {
                        this.setBasePrice(((BigDecimal) priceResult.get("price")).divide(pieces,3, RoundingMode.HALF_UP));
                        this.setAlternativeProductPrice((BigDecimal) priceResult.get("price"));
                    }else{
                    ...
                    if(isAlternativePacking){
                        ...
                        if (priceResult.get("listPrice") != null) {
                            this.listPrice = ((BigDecimal) priceResult.get("listPrice")).divide(pieces, UtilNumber.getBigDecimalScale("order.decimals"), UtilNumber.getBigDecimalRoundingMode("order.rounding"));
                        }

                        if (priceResult.get("basePrice") != null) {
                            this.setBasePrice(((BigDecimal) priceResult.get("basePrice")).divide(pieces, UtilNumber.getBigDecimalScale("order.decimals"), UtilNumber.getBigDecimalRoundingMode("order.rounding")));
                            this.setAlternativeProductPrice((BigDecimal) priceResult.get("basePrice"));
                        }

                        if (priceResult.get("price") != null) {
                            this.setDisplayPrice(((BigDecimal) priceResult.get("price")).divide(pieces, UtilNumber.getBigDecimalScale("order.decimals"), UtilNumber.getBigDecimalRoundingMode("order.rounding")));
                        }

                        if (priceResult.get("specialPromoPrice") != null) {
                            this.setSpecialPromoPrice(((BigDecimal) priceResult.get("specialPromoPrice")).divide(pieces, UtilNumber.getBigDecimalScale("order.decimals"), UtilNumber.getBigDecimalRoundingMode("order.rounding")));
                        }
                    }else{
                        if (priceResult.get("listPrice") != null) {
                    ...
                    if (configWrapper != null) {
                        ...
                        if (useAlternaltiveUomQty()) {
                            BigDecimal totalPrice = configWrapper.getTotalPrice();
                            if (totalPrice != null) {
                                this.setBasePrice((totalPrice).divide(pieces, UtilNumber.getBigDecimalScale("order.decimals"), UtilNumber.getBigDecimalRoundingMode("order.rounding")));
                                this.setAlternativeProductPrice(totalPrice);
                            }
                        }
                    }
...
//        Debug.logInfo("Price" + getBasePrice() + " quantity" +  quantity + " Rental adj:" + getRentalAdjustment() + " other adj:" + getOtherAdjustments(), module);
          if (UtilValidate.isEmpty(this.quantityUomId))
              return getBasePrice().multiply(quantity).multiply(getRentalAdjustment()).add(getOtherAdjustments());
          else
              return getAlternativeProductPrice().multiply(quantity).multiply(getRentalAdjustment()).add(getOtherAdjustments());
    }
    ...
    public BigDecimal getItemSubTotal() {
        if (UtilValidate.isEmpty(this.quantityUomId))
            ...
            return this.getItemSubTotal(this.getAlternativeProductQuantity());
    }
    ...
    public BigDecimal getDisplayItemSubTotal() {
        if (this.quantityUomId == null)
            ...
            return this.getAlternativeProductPrice().multiply(this.getAlternativeProductQuantity()).multiply(this.getRentalAdjustment()).add(this.getOtherAdjustments());
    }
    ...
    public BigDecimal getDisplayItemSubTotalNoAdj() {
        if (this.quantityUomId == null)
            ...
            return this.getAlternativeProductPrice().multiply(this.getAlternativeProductQuantity());
    ...
    public boolean equals(String productId, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, String accommodationMapId, String accommodationSpotId,
            ...
            ProductConfigWrapper configWrapper, String itemType, ShoppingCart.ShoppingCartItemGroup itemGroup, boolean isPromo, String quantityUomId) {
        if((quantityUomId == null && this.quantityUomId == null) || (this.quantityUomId != null && quantityUomId != null && this.quantityUomId.equals(quantityUomId))) {
            if (UtilValidate.isEmpty(itemType)) {
                //Use _product from current cart item. If productId is the same for both cart item, the productTypeId will also be the same
                if (UtilValidate.isNotEmpty(_product.getString("productTypeId"))) {
                    if ("ASSET_USAGE".equals(_product.getString("productTypeId"))) {
                        itemType = "RENTAL_ORDER_ITEM";  // will create additional workeffort/asset usage records
                    } else if ("ASSET_USAGE_OUT_IN".equals(_product.getString("productTypeId"))) {
                        itemType = "RENTAL_ORDER_ITEM";
                    } else {
                        itemType = "PRODUCT_ORDER_ITEM";
                    }
                } else {
                    itemType = "PRODUCT_ORDER_ITEM";
                }
            }
            return equals(productId, reservStart, reservLength, reservPersons, accommodationMapId, accommodationSpotId, additionalProductFeatureAndAppls, attributes, prodCatalogId,selectedAmount, configWrapper, itemType, itemGroup, isPromo);
        } else {
            return false;
        }
    }
