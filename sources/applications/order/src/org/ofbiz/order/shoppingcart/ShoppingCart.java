...
public class ShoppingCart implements Iterable<ShoppingCartItem>, Serializable {
    ...
    public List<GenericValue> getSupplierProductList(String productId, BigDecimal quantity, String quantityUomId, LocalDispatcher dispatcher) {
        List<GenericValue> productSuppliers = null;
        Map<String, Object> params = UtilMisc.<String, Object>toMap("productId", productId,
                                    "partyId", this.getSupplierAgentPartyId(),
                                    "currencyUomId", this.getCurrency(),
                                    "quantity", quantity);
        if (UtilValidate.isNotEmpty(quantityUomId)) {
            params.put("quantityUomId", quantityUomId);
        }
        try {
            Map<String, Object> result = dispatcher.runSync("getSuppliersForProduct", params);
            productSuppliers = UtilGenerics.checkList(result.get("supplierProducts"));
        } catch (Exception e) {
            Debug.logWarning(UtilProperties.getMessage(resource_error,"OrderRunServiceGetSuppliersForProductError", locale) + e.getMessage(), module);
        }
    ...
    public List<GenericValue> getProductSalesProductPrices(String productId, BigDecimal quantity, BigDecimal amount, String productStoreGroupId, String productPricePurposeId, String quantityUomId, String termUomId, LocalDispatcher dispatcher) {
        ...
        try {
            ...
            Debug.logError(e, "Error getting product store info from the database while searching price" + e.toString(), module);
            return null;
        }
        ...
        try {
            ...
            Debug.logError(e, "Error getting product info from the database while searching price" + e.toString(), module);
            return null;
        }
        ...
        if (UtilValidate.isEmpty(partyId) && getUserLogin() != null) {
            GenericValue userLogin = getUserLogin();
            partyId = userLogin.getString("partyId");
        }

        // check for auto-userlogin for price rules
        if (UtilValidate.isEmpty(partyId) && getAutoUserLogin() != null) {
            GenericValue userLogin = getAutoUserLogin();
            partyId = userLogin.getString("partyId");
        }
        ...
        if (UtilValidate.isNotEmpty(quantityUomId)) {
            productPriceEcList.add(EntityCondition.makeCondition("quantityUomId", EntityOperator.EQUALS, quantityUomId));
        }
    ...
    public int addOrIncreaseItem(String productId, BigDecimal selectedAmount, BigDecimal quantity, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons,
            ...
            ProductConfigWrapper configWrapper, String itemType, String itemGroupNumber, String parentProductId, LocalDispatcher dispatcher) throws CartItemModifyException, ItemNotFoundException {
       return addOrIncreaseItem(productId,selectedAmount,quantity,reservStart,reservLength,reservPersons,
               accommodationMapId,accommodationSpotId,shipBeforeDate,shipAfterDate,features,attributes, orderItemAttributes,prodCatalogId,
                configWrapper,itemType,itemGroupNumber,parentProductId, null,dispatcher);
    }

    public int addOrIncreaseItem(String productId, BigDecimal selectedAmount, BigDecimal quantity, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons,
            String accommodationMapId, String accommodationSpotId,
         Timestamp shipBeforeDate, Timestamp shipAfterDate, Map<String, GenericValue> features, Map<String, Object> attributes, Map<String, String> orderItemAttributes, String prodCatalogId,
         ProductConfigWrapper configWrapper, String itemType, String itemGroupNumber, String parentProductId, String quantityUomId, LocalDispatcher dispatcher) throws CartItemModifyException, ItemNotFoundException {
        ...
        for (int i = 0; i < this.cartLines.size(); i++) {
            ...
            if (sci.equals(productId, reservStart, reservLength, reservPersons, accommodationMapId, accommodationSpotId, features, attributes, prodCatalogId,selectedAmount, configWrapper, itemType, itemGroup, false, quantityUomId)) {
                BigDecimal newQuantity = sci.getQuantity().add(quantity);
                try {
                    BigDecimal itemBasePrice = sci.getBasePrice();
                    if (sci.getQuantityUomId() != null) {
                        newQuantity = sci.getAlternativeProductQuantity().add(quantity);
                        itemBasePrice = sci.getAlternativeProductPrice();
                    }
                    BigDecimal minQuantity = ShoppingCart.getMinimumOrderQuantity(delegator, itemBasePrice, sci.getProductId(), quantityUomId);
                ...
                if (UtilValidate.isNotEmpty(quantityUomId)) {
                    BigDecimal initialQuantity = newQuantity;
                    String uomId = sci.getQuantityUomId();
                    String uomIdTo = sci.getProduct().getString("quantityUomId");
                    ...
                    if (getOrderType().equals("PURCHASE_ORDER")) {
                        ...
                        productPriceValues = getSupplierProductList(sci.getProductId(), quantity, uomId, dispatcher);
                    } else if (getOrderType().equals("SALES_ORDER")) {
                        //Check product prices
                        productPriceValues = getProductSalesProductPrices(sci.getProductId(), quantity, null, null, "PURCHASE", uomId, null, dispatcher);
                    }
                    if (UtilValidate.isNotEmpty(productPriceValues)) {
                        ...
                        if (UtilValidate.isNotEmpty(productPriceValue.get("roundingMode"))) {
                            ...
                            sci.setAlternativeRoundingMode(roundingModeStr);
                        }
                        Long scaleLong = null;
                        if (UtilValidate.isNotEmpty(productPriceValue.get("decimalScale"))) {
                            scaleLong = productPriceValue.getLong("decimalScale");
                            sci.setAlternativeDecimalScale(scaleLong);
                        }
                        BigDecimal conversionFactor = null;
                        if (UtilValidate.isNotEmpty(productPriceValue.get("piecesIncluded"))) {
                            BigDecimal piecesIncluded = productPriceValue.getBigDecimal("piecesIncluded");
                            conversionFactor = piecesIncluded;
                            sci.setAlternativePiecesIncluded(piecesIncluded);
                        }
                        newQuantity = UomWorker.convertStockUom(newQuantity, conversionFactor, roundingModeStr, scaleLong, uomId, uomIdTo, dispatcher);
                    }
                    sci.setQuantity(newQuantity, dispatcher, this, true, true, false, false);
                    sci.setAlternativeProductQuantity(initialQuantity);
                } else {
        ...
        if (getOrderType().equals("PURCHASE_ORDER")) {
            ...
            if (supplierProduct != null || "_NA_".equals(this.getPartyId())) {
                 item = ShoppingCartItem.makePurchaseOrderItem(Integer.valueOf(0), productId, selectedAmount, quantity, features, attributes, prodCatalogId, configWrapper, itemType, itemGroup, dispatcher, this, supplierProduct, shipBeforeDate, shipAfterDate, cancelBackOrderDate, quantityUomId);
            } else {
                throw new CartItemModifyException("SupplierProduct not found");
            }
        } else {
            try {
                BigDecimal minQuantity = ShoppingCart.getMinimumOrderQuantity(delegator, null, productId, quantityUomId);
            ...
            item = ShoppingCartItem.makeItem(Integer.valueOf(0), productId, selectedAmount, quantity, null,
                    ...
                    this, Boolean.TRUE, Boolean.TRUE, parentProductId, quantityUomId, Boolean.FALSE, Boolean.FALSE);
        }
    ...
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, HashMap<String, GenericValue> features, HashMap<String, Object> attributes, String prodCatalogId, String itemType, ProductConfigWrapper configWrapper, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules) throws CartItemModifyException, ItemNotFoundException {
        return addItemToEnd(ShoppingCartItem.makeItem(null, productId, amount, quantity, unitPrice, null, null, null, null, null, features, attributes, prodCatalogId, configWrapper, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, null, Boolean.FALSE, Boolean.FALSE));
    }
    ...
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, HashMap<String, GenericValue> features, HashMap<String, Object> attributes, String prodCatalogId, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules) throws CartItemModifyException, ItemNotFoundException {
        return addItemToEnd(ShoppingCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart, reservLength, reservPersons, null, null, features, attributes, prodCatalogId, null, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, null, Boolean.FALSE, Boolean.FALSE));
    }
    ...
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, HashMap<String, GenericValue> features, HashMap<String, Object> attributes, String prodCatalogId, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules, Boolean skipInventoryChecks, Boolean skipProductChecks) throws CartItemModifyException, ItemNotFoundException {
        return addItemToEnd(ShoppingCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart, reservLength, reservPersons, null, null, features, attributes, prodCatalogId, null, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, null, skipInventoryChecks, skipProductChecks));
    }
    ...
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, HashMap<String, GenericValue> features, HashMap<String, Object> attributes, String prodCatalogId, ProductConfigWrapper configWrapper, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules, Boolean skipInventoryChecks, Boolean skipProductChecks) throws CartItemModifyException, ItemNotFoundException {
        return addItemToEnd(ShoppingCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart, reservLength, reservPersons, null, null, features, attributes, prodCatalogId, configWrapper, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, null, skipInventoryChecks, skipProductChecks));
    }
    ...
    /** Add an item to the shopping cart. */
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, HashMap<String, GenericValue> features, HashMap<String, Object> attributes, String prodCatalogId, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules, Boolean skipInventoryChecks, Boolean skipProductChecks) throws CartItemModifyException, ItemNotFoundException {
        return addItemToEnd(ShoppingCartItem.makeItem(null, productId, amount, quantity, unitPrice, null, null, null, null, null, features, attributes, prodCatalogId, null, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, null, skipInventoryChecks, skipProductChecks));
    }

    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersonsDbl,String accommodationMapId, String accommodationSpotId, HashMap<String, GenericValue> features, HashMap<String, Object> attributes, String prodCatalogId, ProductConfigWrapper configWrapper, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules, String quantityUomId,Boolean skipInventoryChecks, Boolean skipProductChecks) throws CartItemModifyException, ItemNotFoundException {
        return addItemToEnd(ShoppingCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart, reservLength, reservPersonsDbl, accommodationMapId, accommodationSpotId, null, null, features, attributes, prodCatalogId, configWrapper, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, quantityUomId, skipInventoryChecks, skipProductChecks));
    }
...
//    protected String billFromVendorPartyId = null;
  //  protected String shipFromVendorPartyId = null;
    ...
    public void positionItemToGroup(ShoppingCartItem item, BigDecimal quantity, int fromIndex, int toIndex, boolean clearEmptyGroups) {
        ...
        if (item.useAlternaltiveUomQty()) {
            BigDecimal conversionFactor = item.getAlternativeConversionFactor();
    ...
    public List<GenericValue> makeOrderItems(boolean explodeItems, boolean replaceAggregatedId, LocalDispatcher dispatcher) {
        ...
        synchronized (cartLines) {
            ...
            for (ShoppingCartItem item : cartLines) {
                ...
                orderItem.set("quantityUomId", item.getQuantityUomId());
                orderItem.set("altUnitPrice", item.getAlternativeProductPrice());
                orderItem.set("altQuantity", item.getAlternativeProductQuantity());
                orderItem.set("altPiecesIncluded", item.getAlternativePiecesIncluded());
                orderItem.set("decimalScale", item.getAlternativeDecimalScale());
                orderItem.set("roundingMode", item.getAlternativeRoundingMode());
    ...
    public static BigDecimal getMinimumOrderQuantity(Delegator delegator, BigDecimal itemBasePrice, String itemProductId, String quantityUomId) throws GenericEntityException {
        ...
        if (UtilValidate.isNotEmpty(quantityUomId)) {
            exprs.add(EntityCondition.makeCondition("quantityUomId", EntityOperator.EQUALS, quantityUomId));
        }
        ...
        if (itemBasePrice == null) {
            ...
            if (UtilValidate.isNotEmpty(productPriceList) && UtilValidate.isNotEmpty(quantityUomId)) {
                productPriceList = EntityUtil.filterByCondition(productPriceList, EntityCondition.makeCondition("quantityUomId", EntityOperator.EQUALS, quantityUomId));
            }
