    ...
    public static Map<String, Object>loadCartFromOrder(DispatchContext dctx, Map<String, Object> context) {
        ...
        if (UtilValidate.isNotEmpty(orderItems)) {
            for (GenericValue item : orderItems) {
                ...
                if (item.get("productId") == null) {
                    ...
                    try {
                        ...
                        if (UtilValidate.isNotEmpty(quantityUomId)) {
                            quantity = item.getBigDecimal("altQuantity");
                        }
                        itemIndex = cart.addItemToEnd(productId, amount, quantity, unitPrice, reservStart, reservLength, reservPersons,accommodationMapId,accommodationSpotId, null, null, prodCatalogId, configWrapper, item.getString("orderItemTypeId"), dispatcher, null, unitPrice == null ? null : false, quantityUomId, skipInventoryChecks, skipProductChecks);
                ...
                cartItem.setQuantityUomId(item.getString("quantityUomId"));
                cartItem.setAlternativeProductQuantity(item.getBigDecimal("altQuantity"));
                cartItem.setAlternativeProductPrice(item.getBigDecimal("altUnitPrice"));
                cartItem.setAlternativePiecesIncluded(item.getBigDecimal("altPiecesIncluded"));
                if (UtilValidate.isNotEmpty(item.get("decimalScale"))){
                    cartItem.setAlternativeDecimalScale(item.getLong("decimalScale"));
                }
                if (UtilValidate.isNotEmpty(item.get("roundingMode"))){
                    cartItem.setAlternativeRoundingMode(item.getString("roundingMode"));
                }
