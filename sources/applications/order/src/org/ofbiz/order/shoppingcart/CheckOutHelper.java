...
public class CheckOutHelper {
    ...
    private Map<String, Object> makeTaxContext(int shipGroup, GenericValue shipAddress, Map<Integer, ShoppingCartItem> shoppingCartItemIndexMap, String originFacilityId, boolean skipEmptyAddresses) {
        ...
        List<String> quantityUom = new ArrayList<String>(totalItems);
        ...
        for (int i = 0; i < totalItems; i++) {
            ...
            if (cartItem.getQuantityUomId() != null) {
                cartItem.getAlternativeConversionFactor();
                BigDecimal itemQuantity = itemInfo.quantity;
                BigDecimal conversionFactor = cartItem.getAlternativeConversionFactor();
                if (UtilValidate.isEmpty(conversionFactor)) {
                    conversionFactor = BigDecimal.ONE;
                }
                String roundingModeStr = cartItem.getAlternativeRoundingMode();
                ...
                Long decimalScaleLong = cartItem.getAlternativeDecimalScale();
                ...
                itemQuantity = itemInfo.quantity.divide(conversionFactor, decimalScale, roundingMode);
                amount.add(i, cartItem.getItemSubTotal(itemQuantity));
                price.add(i, cartItem.getAlternativeProductPrice());
                quantity.add(i, cartItem.getAlternativeProductQuantity());
                quantityUom.add(i, cartItem.getQuantityUomId());
            } else {
        ...
        serviceContext.put("itemQuantityUomIdList", quantityUom);
